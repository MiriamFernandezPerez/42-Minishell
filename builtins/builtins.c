...
int	make_exit(t_tokens **str, t_data *data)
{
	...
	if (str[1]->value) // si 2 args et plus
	{
		...
		{
			write(2, "exit\n👯 minishell> : exit: ", 29);
			write(2, &(*str[1]->value), ft_strlen(str[1]->value));
...
void	make_echo(t_tokens **str) // avec char null a la fin
{
	if (str[1])
		write (1, str[1]->value, ft_strlen(str[1]->value));
	if (ft_strncmp(str[0]->value, "echo -n", 7) != 0)
		write(1, "\n", 1);
	return ;
}
